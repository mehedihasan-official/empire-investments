      ...
      <main className="min-h-screen pt-16 bg-navy-900 flex">
        ...
        <div className="flex-1 lg:ml-64">
          <div className="p-4 sm:p-6 lg:p-8">
            ...
            <p className="mb-6 text-sm text-gray-400 sm:text-base">Manage and control user accounts</p>
            ...
            <div className="bg-navy-800 border border-gold-500/20 rounded-lg overflow-hidden">
              ...
              {!loading && users.length > 0 && (
                <div className="space-y-3 p-3 md:hidden">
                  {users.map((u) => (
                    <div key={u._id} className="rounded-lg border border-gold-500/20 bg-navy-700/40 p-4">
                      <div className="mb-2 text-white font-semibold">{u.displayName}</div>
                      <div className="mb-3 text-sm text-gray-300 break-all">{u.email}</div>
                      <div className="mb-3">
                        <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-400">Role</label>
                        <select
                          value={u.role}
                          onChange={(e) => updateUserRole(u._id, e.target.value)}
                          className="w-full px-3 py-2 bg-navy-700 border border-gray-600 text-white rounded text-sm focus:outline-none focus:border-gold-400"
                        >
                          <option value="user">User</option>
                          <option value="admin">Admin</option>
                        </select>
              ...
              {loading && (
                <div className="p-6 text-center text-gray-400 md:hidden">Loading users...</div>
              )}
              {!loading && users.length === 0 && (
                <div className="p-6 text-center text-gray-400 md:hidden">No users found</div>
              )}
            </div>
